...
### Libraries
We expect you to use `cxxopts` for parsing command-line options, and `spdlog` for variable-level logging. We will require certain logs to be printed using `spdlog` from both the HTTP proxy and the load balancer in order to faciliate autograding and debugging. 
...
In tutorial 04, we covered an example of using **select polling** to handle multiple incoming client sockets. You should go over the slides, and look through the example code [here](https://github.com/henryhxu/CSCI4430/tree/2025_fall/tutorial/T04/select_example). `miProxy` should handle multiple client sockets in much the same way. 
