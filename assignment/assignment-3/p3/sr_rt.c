...
struct in_addr gw, struct in_addr mask,char* if_name)
{
    ...
    rt_walker->next = (struct sr_rt*)tracked_malloc(sizeof(struct sr_rt));
    ...
    rt_walker->gw   = gw;
    rt_walker->mask = mask;
    strncpy(rt_walker->interface,if_name,IF_NAMESIZE);
